...
test.each(["success", "disconnect", "conflict", "forbidden", "invalid", "exhausted"])(
  "atomic screenshot upload: %s",
  async (mode) => {
    ...
    const server = createServer(async (req, res) => {
      ...
      if (req.method === "GET" && req.url === "/repos/owner/images")
        ...
        if (mode === "forbidden" || mode === "invalid") {
    ...
    try {
      ...
      if (["forbidden", "invalid", "exhausted"].includes(mode))
