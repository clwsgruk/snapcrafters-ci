...
export function measure(texts: string[], lines: number, files: number) {
  const count = texts.reduce((total, text) => {
    const nonblank = text.split(/\r?\n/).filter((line) => line.trim());
    return total + nonblank.length;
  }, 0);

  if (count > lines) {
    throw Error(`line cap exceeded: ${count} > ${lines}`);
  }
  if (texts.length > files) {
    throw Error(`file cap exceeded: ${texts.length} > ${files}`);
  }
