...
if (generated.length !== 24) throw Error("Expected 12 bundles and 12 license notices");
const hash = (bytes: Buffer) => createHash("sha256").update(bytes).digest("hex");
const expected = new Map(
  generated.map((p) => [
    p,
    hash(execFileSync("git", ["show", `HEAD:${p}`], { maxBuffer: 16 * 1024 * 1024 })),
  ]),
);
for (const file of tracked.filter((p) => /^(src\/|[^/]+\/main\.ts$)/.test(p)))
  if (/process\.env\.(NODE_ENV|CI_SMOKE|TEST_MODE)|smoke.?bypass/i.test(readFileSync(file, "utf8")))
