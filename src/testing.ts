...
const table = (rows: Manifest[]) =>
  `<table><thead><tr><th>CPU Architecture</th><th>Revision</th></tr></thead><tbody>${rows.map((r) => `<tr><td>${r.architecture}</td><td>${r.revision}</td></tr>`).join("")}</tbody></table>`;
...
export function testingIssue(
  ...
  if (body.length > 100000) throw Error("Testing issue too large");
  const embedded = [...body.matchAll(/<!-- snapcrafters-testing:(.+) -->/g)];
  if (embedded.length > 1) throw Error("Ambiguous testing context");
  let data: Testing;
  if (embedded.length) data = JSON.parse(embedded[0][1]) as Testing;
  else {
