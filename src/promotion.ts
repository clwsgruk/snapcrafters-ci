...
export async function promote(
  ...
  const issueId = revision(String(event.issue.number)),
    commentId = revision(String(event.comment.id)),
    issuePath = `/repos/${repo}/issues/${issueId}`;
  ...
  const active = (arch: string, rev: string, target: string) =>
    revisions(command("snapcraft", ["revisions", snap, "--arch", arch], process.cwd(), env)).some(
      (r) =>
...
export async function promotionAction() {
  ...
  if (statSync(eventFile).size > 1024 * 1024) throw Error("Event too large");
  return promote(
